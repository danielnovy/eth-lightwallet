...
multiEncryptSymKey = function(symEncryptionKey, theirPubKeyArray) {
  ...
  for (var i=0; i < theirPubKeyArray.length; i++) {
    var nonce = nacl.randomBytes(nacl.box.nonceLength);
    var theirPublicKey = nacl_decodeHex(theirPubKeyArray[i]);
    var encSymKeyMsg = nacl.box(symEncryptionKey, nonce, theirPublicKey, ephemeralKeyPair.secretKey);

    var encSymKey = {ciphertext: nacl.util.encodeBase64(encSymKeyMsg),
                     nonce: nacl.util.encodeBase64(nonce),
                     ephemeralPubKey: nacl_encodeHex(ephemeralKeyPair.publicKey)};
...
multiEncryptString = function (msg, theirPubKeyArray) {
  ...
  console.log('SYN_ENCRYPTION_KEY: [' + (symEncryptionKey) + ']');
  ...
  output.ephemeralPubKey = nacl_encodeHex(ephemeralKeyPair.publicKey);

  return output;
}

multiDecryptString = function (keystore, pwDerivedKey, encMsg, myPubKey, hdPathString) {

  var symKey = false;
  for (var i=0; i < encMsg.encryptedSymKey.length; i++) {
    var ephemPubKey = encMsg.ephemeralPubKey;
    var result = _asymDecryptRaw(keystore, pwDerivedKey, encMsg.encryptedSymKey[i], ephemPubKey, myPubKey, hdPathString)
    if (result !== false) {
      symKey = result;
      break;
    }
...
module.exports = {
  ...
  asymDecryptRaw: _asymDecryptRaw,
  multiEncryptSymKey: multiEncryptSymKey
};
